...
<script type="text/javascript">
   function promot() {//提示添加成功		
	...
	<body onload="promot()">
		<form action="${pageContext.request.contextPath }/admin/addUserInfo"  name="form1" onsubmit="return validator(this)"   method="post"
				...
				<tr>
					...
					<td colspan="3" bgcolor="#FFFFFF">
						<select name="departmentId" style="width: 145px">
						  <c:forEach items="${ deptinfo}" var="di">
							<option value="${di.departmentId }">${di.departmentName}</option>					
					    </c:forEach>
					...
					<td colspan="3" bgcolor="#FFFFFF">
						<select name="roleId" style="width: 145px">
						  ...
						  <c:forEach items="${userrole }" var="ur">
							<option value="${ur.roleId }">${ur.roleName}</option>					
					    </c:forEach>
						</select>
						&nbsp;
					</td>
